    ...
    private void initView(JSONObject jsonobj){
        try{
            ...
            if (property_list.length() > 0){
                ...
                object.setText(property_list.getJSONObject(0).getString("PROPERTY_NAME"));
                TextView area = findViewById(R.id.property_area);
                TextView pro_price = findViewById(R.id.property_price);
                for (int j = 0 ; j<property_list.getJSONObject(0).getJSONArray("PROPERTY_PRICES").length(); j++) {
                    JSONObject price = property_list.getJSONObject(0).getJSONArray("PROPERTY_PRICES").getJSONObject(j);
                    if (price.getString("PRICE_TYPE").equals("40004002")) {
                        area.setText("面积：" + price.getString("AREA"));
                        pro_price.setText("价格：" + price.getString("TOTAL_PRICE") + "万元");
                    }
                }
            }
